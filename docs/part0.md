...
Most developer portfolios are a scrollable page: a header, a grid of projects, a contact form. They're fine. But they all blur together. Over this series we're going to build something a visitor *remembers* — a **3D room you can walk through**. A desk with a monitor you can click to open projects. A wall of diplomas. A TV that plays your showreel. Lights you can flip. A guided tour. All of it in the browser, all of it from scratch.
...
By the end of the series you'll have a single, cohesive scene that *is* your portfolio:
...
- 🖥️ A **clickable desktop inside the monitor** ("Hadi OS") with project windows and a résumé timeline, plus a playable TV and a gallery wall of your credentials
...
We keep the stack small and modern:

- **React + TypeScript + Vite** — the app shell and a fast dev loop
- **[React Three Fiber](https://r3f.docs.pmnd.rs/)** — Three.js written as React components
- **[drei](https://github.com/pmndrs/drei)** — the helper belt (`useGLTF`, `Html`, `CameraControls`, `Environment`, and more)
- **[@react-three/postprocessing](https://github.com/pmndrs/react-postprocessing)** — Bloom, so lights glow
- **[zustand](https://github.com/pmndrs/zustand)** — shared state that works *both* inside and outside the 3D canvas
- **[Three.js](https://threejs.org/)** — under the hood, when we need it directly
...
Thirteen parts, each one focused, each building on the last. They group into three acts.
...
- **Follow in order.** Each part assumes the one before it — the room grows step by step.
- **Every part has the full code.** Each tutorial ends with a link to its branch on [GitHub](https://github.com/myoxine/3d-profile), so you can check out the exact state for that step.
- **It's honest about the hard parts.** Where a choice bit us — a model that mangled under compression, a camera that escaped the room, eyes on the back of an avatar's head — the tutorial tells you what went wrong and why, not just the clean final answer.
...
Let's build something people remember. **[Start with Part 1 →](part1.md)**

---

## Source Code

👉 The complete project lives on [GitHub](https://github.com/myoxine/3d-profile); each part has its code on a matching `partN` branch.
