# 3D Profile Website - Part 9: Make It Interactive — A Desktop-in-the-Monitor, Hover Tooltips, an Auto Tour & Deep Links
...
| Approach | Clickable? | Occludes correctly? |
|---|---|---|
| **Textured mesh** (an image on a `planeGeometry`) | ❌ no DOM events | ✅ real geometry — hidden behind the character, sorted by depth |
| **`<Html transform>`** (real DOM projected onto the surface) | ✅ full HTML/CSS | ⚠️ it's an overlay — it bleeds *over* geometry in front of it |
...
> 💡 `toneMapped={false}` keeps the screenshot at full brightness regardless of scene lighting/Bloom — exactly what a glowing screen should do. We use the same trick for the **laptop screen** ([LaptopScreen.tsx](../src/components/LaptopScreen.tsx)), which is just a `meshBasicMaterial` plane showing a VS Code screenshot at a hand-calibrated `0.405 × 0.255 m`.
...
export type Project = {
  ...
  role: string; desc: string; tech: string[]; highlights: string[]
}
...
```tsx
{open && (
  <div /* absolutely-positioned window */>
    <div /* title bar with ●●● dots */>{open.icon} {open.name}</div>
    <p>{open.desc}</p>
    <ul>{open.highlights.map((h,i) => <li key={i}>{h}</li>)}</ul>
    <div>{open.tech.map((t) => <span key={t}>{t}</span>)}</div>
  </div>
)}
```
...
```tsx
{resume && (
  <ol style={{ borderLeft: '2px solid rgba(255,210,122,0.4)' }}>
    {EXPERIENCE.map((e, i) => (
      <li key={i}>
        <span /* glowing node on the line */ />
        <div>{e.role}</div>
        <div>{e.company} · {e.period}</div>
        <div>{e.desc}</div>
        {e.highlights && <ul>{e.highlights.map((h,j) => <li key={j}>{h}</li>)}</ul>}
      </li>
    ))}
  </ol>
)}
...
> 🗂️ **Two configs, one screen.** `projects.ts` feeds the desktop icons; `experience.ts` feeds the Resume app. Both are plain data — the 3D code never changes when your CV does.
...
- **Place the tooltip over the object's body, not its top edge.** The book tooltip vanished when the pointer moved off the spine; lowering it to sit over the book body (`bottomY + halfH*1.92`) kept hover stable.

The light switches get the same treatment with a friendly label + on/off status, so hovering "Lampu Meja" tells you what it controls before you flip it:

```tsx
// src/components/LightSwitch.tsx  (labels: Lampu Plafon, Neon Sign, Lampu Meja, Lampu Sofa)
{hovered && (
  <Html position={[0, 0.085, 0.02]} center style={{ pointerEvents: 'none' }}>
    <div>💡 {label} — {on ? 'NYALA' : 'MATI'}</div>
  </Html>
)}
```
...
const dwellOf = (v: ViewName) => (v === 'overview' ? 11000 : 8000) // Beranda reads longer
...
Two `useEffect` timers, **both cleaned up** on every step change — that cleanup is what makes manual **Next / Previous** work without leaking a stale auto-advance. The button label even doubles as the countdown:
...
const VIEW_INFO: Record<ViewName, { icon: string; title: string; desc: string; points?: string[] }> = {
  desk: {
    icon: '🚀', title: 'Experience & Portfolio',
    desc: 'Layar monitor menampilkan "Hadi OS" — desktop interaktif berisi karier & proyek.',
    points: [
      '💼 Ikon Resume — buka timeline 7 pengalaman kerja …',
      '🗂️ 12 ikon proyek dari CV — klik untuk detail …',
...
We want `…/#experience` to open straight on the desk, and we want every navigation to update the URL so any framing is bookmarkable. A small hook maps views ↔ slugs in both directions:
...
- **The URL hash makes a 3D scene shareable.** Map views ↔ slugs, sync with `replaceState`, guard the loop.
...
Thanks for following along — go make your room *talk back*. 🖥️
