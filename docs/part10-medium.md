...
The tool is glTF-Transform, run via npx — no install. But the exact command matters enormously, and the obvious one will crash your app. Here’s the one we actually shipped:
...
That’s a lot of false. Every one of those flags turns off a pass that optimize runs by default — and each of those default passes can break a scene built from gltfjsx components. Step 2 is the whole story; for now, trust the flags.
...
So nodes.Cube006_Cube007 becomes undefined, and .geometry throws. The fix is to disable every pass that renames or merges named things — exactly the wall of false flags in Step 1.

What’s safe to keep on:

• meshopt / quantization — repacks vertex data, keeps the mesh and its name.
• weld — merges duplicate vertices within a primitive. It changes vertex counts, not node names, so gltfjsx lookups still resolve. (Leaving weld on is what got the sofa from 6.6 MB back down to 1.6 MB — a big chunk of the win.)
