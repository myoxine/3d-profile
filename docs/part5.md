...
By now you have a fully enclosed room — walls, floor, a shiny window, a door, a ceiling, real furniture, and beautiful HDRI lighting. It already feels like a real space. But it's still *static*: the lights are always the same, and visitors can only look, not interact.

In **Part 5**, we make the room come alive. This part is all about **light and interactivity**:

✅ A central lighting "brain" that any component can read and control
✅ **Interactive 3D light switches** — click a switch and the rocker physically flips
...
✅ More cozy props: a sofa, an air conditioner, a credenza, and a plant

By the end, your visitors will be able to walk in, flip the switches, and watch the room respond.
...
A 3D scene with baked, unchanging light is just a picture you can orbit around. The moment a user can **change** something — flip a switch, see a lamp glow, watch the room shift from day to night — it becomes an *experience*.
...
> 🔑 Any mesh you want to glow should use `toneMapped={false}` on its emissive material and an `emissiveIntensity` above the Bloom threshold.
...
Three switches sit side by side next to the door — one for each light.
...
export function Ceiling({ size = [3, 3], color = '#dCEBFF', ...props }) {
  ...
  const led = (
    <meshStandardMaterial
      color={color}
      emissive={color}
      emissiveIntensity={ceilingOn ? 2 : 0}
      toneMapped={false}
    />
  )
...
Both the **standing lamp** and the **table lamp** are GLB models. The trick is the same for each:
...
The **table lamp** follows the exact same pattern with `tableOn`, sitting on top of the credenza.

> ⚙️ **Tuning brightness.** Real-time lighting is all about taste. If a lamp is too bright, lower its `pointLight` `intensity` and the bulb's `emissiveIntensity`. If the glow is too strong, raise the Bloom `luminanceThreshold` or lower its `intensity`.

---

## Step 8 — Furnish the Room

To make the space feel lived-in, we drop in a few more GLB props. Each is converted with `gltfjsx` and placed in `Room.tsx`:

```tsx
// src/components/Room.tsx (excerpt)
<Sofa scale={0.4} position={[-0.6, -1.9, -1.0]} />
<StandingLamp position={[-1.3, -1.9, -1.3]} />
<AirConditioner position={[-0.5, 1.2, -1.4]} />     {/* on the back wall, above the sofa */}
...
> 🪶 **Performance tip:** Watch your `.glb` file sizes! A single 90 MB sofa will make your site crawl. Prefer low-poly models, or optimize with Draco compression / texture resizing. Swapping one bloated model for a 20 KB low-poly couch made a huge difference in this project.
...
- **Sofa (Couch Small):** [Poly Pizza](https://poly.pizza/m/ZOPP3KzNIk)
- **Standing lamp:** [CGTrader](https://www.cgtrader.com/items/3727623/download-page)
- **Table lamp:** [CGTrader](https://www.cgtrader.com/items/7091263/download-page)
- **Air conditioner:** [CGTrader](https://www.cgtrader.com/items/5800337/download-page)
- **Credenza (accent chest):** [CGTrader](https://www.cgtrader.com/free-3d-models/furniture/kitchen-cabinet/accent-chest)
...
Clone it, explore it, and make it your own!
