# 3D Profile Website - Part 10: Make It Fast — Cutting 172 MB Down to 44 MB
...
The tool is **[glTF-Transform](https://gltf-transform.dev/)**, run via `npx` — no install. Here's the command we *shipped* — but every flag on it is a scar from something that went wrong first:
...
That's a lot of `false`. Every one turns *off* a pass `optimize` runs by default, and each of those passes can **break a scene built from [gltfjsx](https://github.com/pmndrs/gltfjsx) components**. Two deliberate choices stand out, and both are the *opposite* of the obvious answer:
...
**Compression isn't always a win.** The potted plant came out *bigger*. Tiny meshes with already-small textures don't benefit — the compression's overhead costs more than it saves. **Compare sizes and keep the smaller file.**
